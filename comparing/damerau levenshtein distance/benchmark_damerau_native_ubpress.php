...
$file_output_csv = 'hasil_benchmark_damerau_native_ubpress.csv';
...
function native_damerau_levenshtein(string $str1, string $str2): int
{
    ...
    for ($i = 1; $i <= $len1; $i++) {
        for ($j = 1; $j <= $len2; $j++) {
            ...
            $d[$i][$j] = min(
                $d[$i][$j - 1] + 1,          // 1. Insertion (Penyisipan)
                $d[$i - 1][$j] + 1,          // 2. Deletion (Penghapusan)
                $d[$i - 1][$j - 1] + $cost   // 3. Substitution (Penggantian)
            );
            ...
            if ($i > 1 && $j > 1 && $str1_chars[$i - 1] === $str2_chars[$j - 2] && $str1_chars[$i - 2] === $str2_chars[$j - 1]) {
                $d[$i][$j] = min(
                    $d[$i][$j],
                    $d[$i - 2][$j - 2] + 1 // 4. Transposition (Pertukaran Posisi)
                );
            }
        }
    }
...
echo "Memulai benchmark (Metode Damerau-Levenshtein Native)... Ini akan SANGAT LAMA.\n";
$judul_ke = 0;
foreach ($titles_for_testing as $judul) {
    ...
    for ($i = 0; $i < $jumlah_pengujian_per_judul; $i++) {
        ...
        // --- INTI LOGIKA SEQUENTIAL SEARCH + DAMERAU-LEVENSHTEIN NATIVE ---
        $matches = [];
        foreach ($titles_for_testing as $title_to_scan) {
            ...
            foreach ($queryWords as $queryWord) {
                ...
                foreach ($titleWords as $titleWord) {
                    ...
                    if (native_damerau_levenshtein($queryWord, $titleWord) <= $maxDist) {
                        $foundMatch = true;
                        break;
                    }
                }
