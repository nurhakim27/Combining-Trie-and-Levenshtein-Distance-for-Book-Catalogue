...
// Ganti 'trie-only.php' jika nama file Anda berbeda.
// ==========================================================
require_once 'trie-only.php';
...
$file_output_csv = 'hasil_benchmark_trie_only.csv';

// Ambil Trie yang sudah di-cache agar tidak membangun ulang
$cacheFile = 'trie_cache_pure.dat';
if (!file_exists($cacheFile)) {
    die("File cache '{$cacheFile}' tidak ditemukan. Jalankan dulu skrip trie-only.php sekali dari browser untuk membuatnya.");
}
...
foreach ($daftar_judul_tes as $judul) {
    ...
    for ($i = 0; $i < $jumlah_pengujian_per_judul; $i++) {
        ...
        // --- INTI LOGIKA PENCARIAN (disalin dari trie-only.php) ---
        ...
        if (!empty($queryWords)) {
            ...
            $lastWordResults = $trie->search($lastWord); // Nama fungsi search sudah benar
