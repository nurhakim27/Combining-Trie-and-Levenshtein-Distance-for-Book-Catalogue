...
if (isset($_GET['query'])) {
    ...
    // PENGECEKAN APAKAH ADA DATA YANG BERTAMBAH DI DATABASE? KALAU ADA MAKA DIA AKAN MEMBUAT ULANG CACHE
    $cacheFile = 'trie_cache_ubpress.dat';
    $hashFile = 'trie_cache_ubpress.hash';

    // Ambil semua judul untuk hashing
    $titlesForHash = getTitlesFromDB($dbHost, $dbUser, $dbPass, $dbName, $dbTable);
    $currentHash = md5(implode('|', $titlesForHash));

    // Cek apakah hash sudah ada dan sama
    $cacheValid = false;
    ...
    if ($cacheValid) {
        ...
        // Bangun ulang Trie dan simpan cache + hash
        $trie = new Trie();
        foreach ($titlesForHash as $title) {
            $trie->insert($title);
        }
        file_put_contents($cacheFile, serialize($trie));
        file_put_contents($hashFile, $currentHash);
    }
    // PENGECEKAN APAKAH ADA DATA YANG BERTAMBAH DI DATABASE? KALAU ADA MAKA DIA AKAN MEMBUAT ULANG CACHE (END)
